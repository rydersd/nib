...
- `project-data.js` sets globals on `window` (see below)
- `proto-nav.js` reads those globals and builds the page chrome — context bar, drawer, design notes panel, story mode
- `proto-compose.js` (optional) reads `window.COMPOSE` and transforms it into `window.PAGE_BLUEPRINT`, registering compose-specific block types on DOMContentLoaded — see [[Page-Compose]]
- `proto-gen.js` (optional) reads `window.PAGE_BLUEPRINT` and generates page content — runs on `DOMContentLoaded`, after `proto-nav.js` has executed — see [[Page-Blueprint]]

### `wfNavInit()` sequence

The `proto-nav.js` entry point runs, in order:
...
7. `buildDesignNotesPanel()` — tabbed design notes overlay
8. `buildFeedbackPanel()` — feedback/annotation overlay
9. `buildSettingsPanel()` — theme/settings configuration panel
10. `wfThemeDetect()` — resolve and apply design system theme for the current page
...
18. `wfInitScatterTransition()` — scatter GL page-transition setup
...
| `STORY_MAP` | No | Maps page filenames to story IDs — auto-injects story badges into the Notes Context tab. When `DESIGN_STORIES` is also defined, badges link to `design-stories.html` |
| `STORY_TITLES` | No | Maps story IDs to human-readable titles |
| `JOURNEYS` | No | Array or object format (normalized at init by `normalizeJourneys()`). Drives `data-journey` highlighting across pages |
| `SCENARIOS` | No | Guided walkthroughs — array of `{ id, label, steps: [{ file, narrative, friction? }] }` |
| `DESIGN_STORIES` | No | Rich story objects — user stories, acceptance criteria, phased implementation, decisions, SFDC suggestions. Living design document source of truth |
| `PROJECT_PHASES` | No | Project-level phase definitions — groups stories into delivery phases with system dependencies |
| `PAGE_BLUEPRINT` | No | Declarative page definition — consumed by `proto-gen.js` — see [[Page-Blueprint]] |
...
See [[Navigation]] for the `SECTIONS`, journeys, and scenarios schemas in full.

## CSS module ownership

Each core CSS file owns a specific domain. Never put a style in the wrong module.
...
| `proto-components.css` | UI primitives — see [[Components]] | `.btn`, `.wf-card`, `.wf-table`, `.wf-form-group`, `.wf-badge`, `.wf-tabs` |
| `proto-blueprint.css` | [[Paper-Utilities|Paper aesthetic effects]] | `.wf-tape`, `.wf-pin`, `.wf-torn-*`, `.wf-stacked`, `.wf-sketch`, `[data-wf-confidence]` |
...
Surface CSS (in `surfaces/`, one per page) — see [[Surfaces]]:
...
The page author provides one of:
...
2. `wfDnOpen()` reads the hidden div, splits content by `<h3>` headings into tabs (Context / Design / Technical)
3. Auto-injects story badges from `STORY_MAP` into the Context tab; with `DESIGN_STORIES` defined, badges link to `design-stories.html#story-{id}`
4. CSS: `html.wf-dn-open` shifts body content right 400px; the panel renders at z-index 3000

`proto-gen.js` can also generate notes from the `PAGE_BLUEPRINT.notes` object (summary, jtbd, designSpec, technical, acceptance). See [[Design-Notes]].
...
Framework features toggle classes and attributes on the root element. Your CSS can react:

| State | Applied to `<html>` | Triggered by |
|-------|---------------------|--------------|
| `.wireframe` | Set in HTML boilerplate | Page author (required) |
| `[data-wf-fidelity]` | `"0"`, `"1"`, or `"2"` | Fidelity slider — see [[Fidelity-Levels]] |
...
| `[data-wf-theme]` | Active theme ID (e.g., `"slds"`, `"nib"`) | Theme detection — see [[Themes]] |
...
2. Checks item-level theme, then section-level, then walks backward through `SECTIONS` to find the nearest `isGroup: true` with a `theme`
...
Built-in themes: `nib`, `slds`, `material`, `high-contrast`. Projects add custom themes via `WIREFRAME_CONFIG.themes` — see [[Themes]].

## Key interaction flows

**Hamburger → Drawer:** context bar hamburger button → `wfNavToggle()` → drawer slides in/out, lists pages from `SECTIONS`

**Notes button → Panel:** context bar notes button → `wfDnOpen()` → reads `.wf-design-notes` div → splits by `<h3>` into tabs → injects story badges → panel slides in from right → `html.wf-dn-open` shifts body

**Fidelity slider:** drag slider → `wfFidelity(val)` → sets `data-wf-fidelity` on `<html>` → CSS tokens (`--wf-wobble-radius`, `--wf-grain-opacity`, `--wf-grid-opacity`) respond → `sessionStorage.wf_fidelity` persists

**Scenario walkthrough:** select scenario → `wfScenarioStart(id)` → stores in sessionStorage → navigates to first step → scenario banner shows narrative + friction callouts → Next/Prev buttons advance steps → Exit clears sessionStorage

**Review mode:** toggle Review button → `wfReviewToggle()` → `html.wf-review-active` highlights all `[data-wf-confidence]` elements → hover shows floating toolbar (✓/？/✗) → reaction captured as annotation → stored in sessionStorage + POSTed to `/api/reviews` (Cloudflare KV) → `data-wf-review` attribute applied for visual feedback → Reviews tab in the Notes panel shows summary + export
...
**Block registry** (`proto-gen.js`): register custom block types for `PAGE_BLUEPRINT` via `wfRegisterBlock(type, renderFn)`. Built-in types: detail-grid, related-list, card, kpi-row, timeline, form, table, chart-placeholder, empty-state, accordion, stepper, metric-grid, split-panel, timeline-horizontal.

**Surface plugin registry** (`proto-gen.js`): register custom surface header generators via `wfRegisterSurface(id, { header: fn })`. Built-in surfaces: sfdc, slack, internal.

## REVIEW_ANNOTATIONS structure
...
Stored in sessionStorage as `wf_review_annotations` (full array across pages). Persisted to Cloudflare KV via the `/api/reviews` endpoint. Pulled to git via `nib pull-reviews` → `reviews/{page}-{date}.json`.
...
- [[Review-Mode]] — the annotation workflow the review state serves
