...
const { loadSource } = require('../core/ingest/source');
...
const READ_LABELS = {
  'sheets-api': 'Reading Google Sheets via API',
  'sheets-csv': 'Reading Google Sheets (published CSV)',
  xlsx: 'Reading workbook',
};
...
async function run() {
  ...
  const { tabs, sourceMeta } = await loadSource(args.input, {
    auth: args.auth,
    noun: 'input',
    log: (kind) => log(`${C.cyan}→${C.reset} ${READ_LABELS[kind]}: ${args.input}`),
  });
